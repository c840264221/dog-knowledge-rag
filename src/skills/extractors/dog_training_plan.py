...
TRAINING_GOAL_PATTERNS = [
    # Planner 常把多个明确训练科目放在引号中；它比“制定某种训练计划”
    # 更具体，必须优先匹配，避免把“分阶段”等计划形式误认为训练目标。
    re.compile(
        r"(?:为|围绕)\s*"
        r"(?P<value>"
        r"[“\"「『][^”\"」』]{1,30}[”\"」』]"
    ...
    re.compile(
        r"(?:希望(?:它|狗狗)?|期望(?:它|狗狗)?|想让(?:它|狗狗)?|训练目标(?:是|为)?|目标(?:是|为)?)"
        r"(?P<value>[^。；;]{1,60}?)"
        r"(?=(?:[，,]\s*帮我)|[。；;]|$)"
    ),
    re.compile(
    ...
    # 最后才使用宽泛兜底；它可能命中“分阶段”“个性化”等计划形式词。
    re.compile(
        r"(?:制定|安排|生成)"
        r"(?P<value>[^，。；,;]{1,40}?)"
        r"(?:的)?训练计划"
