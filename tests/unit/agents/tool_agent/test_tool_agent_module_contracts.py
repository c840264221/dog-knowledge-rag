...
功能：
    测试 V1.8 ToolAgent 内部职责地图是否完整、稳定。
...
EXPECTED_MODULE_NAMES = (
    ...
    "tool_execution_runner",
    "tool_result_formatter",
    "tool_debug_report",
)


def test_tool_agent_module_contracts_are_defined() -> None:
    """
    测试 ToolAgent 模块契约已经定义。

    功能：
        确认 get_tool_agent_module_contracts 返回非空契约列表。
    ...
    contracts = get_tool_agent_module_contracts()
...
def test_expected_tool_agent_layers_are_complete() -> None:
    """
    测试 ToolAgent 预期职责层完整。

    功能：
        确认 V1.8 规划中的职责层全部存在，且顺序符合标准执行链路。
    ...
    layers = get_expected_tool_agent_layers()
...
def test_get_tool_agent_contract_by_layer(
    layer: str,
) -> None:
    """
    测试按 layer 获取 ToolAgent 模块契约。

    功能：
        验证 get_tool_agent_contract_by_layer 可以根据层级名称找到对应契约。
    ...
    contract = get_tool_agent_contract_by_layer(
...
def test_get_tool_agent_contract_by_unknown_layer_raises_error() -> None:
    """
    测试未知 layer 会抛出异常。

    功能：
        如果传入不存在的模块层级，get_tool_agent_contract_by_layer 应该抛出 ValueError。
...
def test_render_tool_agent_contract_markdown() -> None:
    """
    测试模块契约 Markdown 渲染。
    ...
    markdown = render_tool_agent_contract_markdown()

    assert "ToolAgent" in markdown
    assert (
        "entry -> intent -> argument -> permission -> execution -> "
        "result_formatter -> debug_report"
    ) in markdown
